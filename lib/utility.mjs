...
export const obtainName = (path, pathAll) => {
  const name = Object.entries(pathAll).find((entry) => entry[1] === path)[0];
 ...
 * @param {string} [scenario] Scenario for multiple API handlers.
 */
export const parseArguments = (scenario) => {
  const options = {},
  ...
  const result = {
    ...
    options,
    params,
    scenario: scenario ?? process.argv[1].split('/').pop(),
  };
